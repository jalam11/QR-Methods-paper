...
for(i in 1:nsims){
  ...
  for(j in 1:length(paras)){
    ...
    for(k in 1:length(taus)){
      ...
      y_dith <- dither(y)
      rq_model_dith <- rq(y_dith~x1 + x2, tau = tau)
      boot_dist <- (boot.rq(cbind(1, x1, x2), y_dith, tau = tau, R = boot))$B[,2]
...
m <- 12
paras <- list(c(0, 100), c(0, 250), c(0, 500),
             c(0.5, 100), c(0.5, 250), c(0.5, 500)) # a list of parameters which I will loop through
taus <- c(0.1, 0.3, 0.5, 0.7, 0.9)
n used were 100, 250, 500
alpha_x1 used were 0, 0.5
mu_x <- 0
sigma_x <- 1
gamma <- 0.1
prob_x2 <- 0.2
...
## July 27-29, 2021 ----
HAD AN ERROR: RNID accidentally not used; RIID used twice. 

z <- qnorm(0.95); z_lower <- 0.05; z_upper <- 0.95; level = 0.9 # For creating 90% Confidence intervals. 

paras <- list( c(1, 100), c(1, 250), # Model 1: IID, normal x and errors
               c(2, 100), c(2, 250), # Model 2: NID, normal x and errors
               c(3, 100), c(3, 250), # Model 3: NID, log-normal x, normal errors
...
## Aug 06, 2021 ----
HAD AN ERROR: Bayesian results not saved
