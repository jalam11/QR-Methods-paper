...
# Notes
- Degenerate intervals ONLY occur when effect estimates are equal to zero

# Version 2 changes (Oct 25, 2021)
- Now loading the gold standard dataset
- Remade the figure 1 to include gold standard, changed the titles

- Jan 17, 2021
-- Changed the titles for all of fig 1a-d
-- Increased font size of x axis 
...
<!-- ```{r} -->
<!-- data_dith <- data_dith %>%  -->
<!--   filter(method != "bayes_adj_v1") -->

<!-- data_dith$method <- data_dith$method %>%  -->
<!--   mapvalues("bayes_adj_v2", "BayesQR (adj)") %>%  -->
<!--   mapvalues("bayes", "BayesQR")  -->
...
data_hist_fqr_dith <- data_hist %>% 
  filter(method_final == "Freq. (Dithered)")

data_hist_fqr_undith <- data_hist %>% 
  filter(method_final == "Freq. (Undithered)")

data_hist_fqr_gs <- data_hist %>% 
  filter(method_final == "Freq. (Gold Standard)")

data_hist_bqr <- data_hist %>% 
  filter(method_final == "Bayesian")
...
png("sim study/results/irreg/hist of beta_mod1_n250_tau50.png",
    height = 6, width = 9, units = "in", res = 300)
par(mfrow=c(2,2))
par(mar = rep(4,4))

# dithered GS
hist(data_hist_fqr_gs$beta[data_hist_fqr_gs$tau == 0.5], breaks = 40,
     xlab = "Effect Estimate",
     main = 'A) Frequentist QR on continuous Y variable without ditherthing',
     cex.main = cex.main)

# undithered freq
hist(data_hist_fqr_undith$beta[data_hist_fqr_undith$tau == 0.5], breaks = 40,
     xlab = "Effect Estimate",
     main = "B) Frequentist QR on count Y variable without ditherthing",
     cex.main = cex.main)

# dithered freq
hist(data_hist_fqr_dith$beta[data_hist_fqr_dith$tau == 0.5], breaks = 40,
     xlab = "Effect Estimate",
     main = "C) Frequentist QR on count Y variable with dithering",
     cex.main = cex.main)

# bayes
hist(data_hist_bqr$beta[data_hist_bqr$tau == 0.5], breaks = 40,
     xlab = "Effect Estimate",
     main = "D) Bayesian QR on count Y variable without ditherthing",
     cex.main = cex.main)
...
png("sim study/results/irreg/hist of beta_mod1_n250_tau10.png",
    height = 6, width = 9, units = "in", res = 300)
par(mfrow=c(2,2))
par(mar = rep(4,4))

# dithered GS
hist(data_hist_fqr_gs$beta[data_hist_fqr_gs$tau == 0.1], breaks = 40,
     xlab = "Effect Estimate",
     main = 'A) Freq. QR on continuous Y variable without ditherthing')

# undithered freq
hist(data_hist_fqr_undith$beta[data_hist_fqr_undith$tau == 0.1], breaks = 40,
     xlab = "Effect Estimate",
     main = "B) Freq. QR on count Y variable without ditherthing")


# dithered freq
hist(data_hist_fqr_dith$beta[data_hist_fqr_dith$tau == 0.1], breaks = 40,
     xlab = "Effect Estimate",
     main = "C) Freq. QR on count Y variable with dithering")

# bayes
hist(data_hist_bqr$beta[data_hist_bqr$tau == 0.1], breaks = 40,
     xlab = "Effect Estimate",
     main = "D) Bayesian QR on count Y variable without ditherthing")
...
png("sim study/results/irreg/hist of beta_mod1_n250_tau90.png",
    height = 6, width = 9, units = "in", res = 300)
par(mfrow=c(2,2))
par(mar = rep(4,4))

# dithered GS
hist(data_hist_fqr_gs$beta[data_hist_fqr_gs$tau == 0.9], breaks = 40,
     xlab = "Effect Estimate",
     main = 'A) Freq. QR on continuous Y variable without ditherthing')

# undithered freq
hist(data_hist_fqr_undith$beta[data_hist_fqr_undith$tau == 0.9], breaks = 40,
     xlab = "Effect Estimate",
     main = "B) Freq. QR on count Y variable without ditherthing")


# dithered freq
hist(data_hist_fqr_dith$beta[data_hist_fqr_dith$tau == 0.9], breaks = 40,
     xlab = "Effect Estimate",
     main = "C) Freq. QR on count Y variable with dithering")

# bayes
hist(data_hist_bqr$beta[data_hist_bqr$tau == 0.9], breaks = 40,
     xlab = "Effect Estimate",
     main = "D) Bayesian QR on count Y variable without ditherthing")
...
hist(data_hist_fqr_undith$beta[data_hist_fqr_undith$tau == 0.5], breaks = 40,
     xlim = c(-0.5, 0.5),
     xlab = "Effect Estimate",
     main = "B) Frequentist QR on Count \nY variable without Ditherthing \n (zoomed in)")
```
...
png("sim study/results/irreg/hist of beta_mod4_n250_tau50.png",
    height = 6, width = 9, units = "in", res = 300)
par(mfrow=c(2,2))
par(mar = rep(4,4))
...
png("sim study/results/irreg/hist of beta_mod4_n250_tau10.png",
    height = 6, width = 9, units = "in", res = 300)
par(mfrow=c(2,2))
par(mar = rep(4,4))
...
png("sim study/results/irreg/hist of beta_mod4_n250_tau90.png",
    height = 6, width = 9, units = "in", res = 300)
par(mfrow=c(2,2))
par(mar = rep(4,4))
