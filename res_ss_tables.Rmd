...
empSE_freq_vs_bayes <- data_short %>% 
  ...
  select(c(n, model, tau, group_reformat, bias, bias_rel)) %>% 
  group_by(n, model, tau, group_reformat) %>% 
  summarise(empSE = round(sqrt( sum((bias_rel)^2) / (sims - 1) ), round),
            empSE_se = round(
              empSE / sqrt( 2  * (sims - 1) ), round_se) 
  ) %>% 
...
empSE_freq_vs_bayes_cor <- data_short %>% 
  ...
  summarise(cor = cor(beta[group_reformat == "Frequentist"], 
           beta[group_reformat == "Bayesian"])) # proves no correlation
...
empSE_freq_vs_bayes_final <- empSE_freq_vs_bayes_final %>% 
  ...
  # mutate(signif_flag = ifelse(p < 0.05,
  #                             T, F))%>%
  # mutate(dif = .[4] - .[5])

rm(empSE_freq_vs_bayes, empSE_freq_vs_bayes_p, p)
...
empSE_count_vs_cnts_cor <- data_short %>% 
  ...
  summarise(cor = cor(beta[status == "Freq. (count)"], 
                       beta[status == "Freq. (cnts)"])) # proves no correlation
...
rm(empSE_count_vs_cnts, empSE_count_vs_cnts_p, p)
