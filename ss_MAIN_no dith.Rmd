...
# simulation
nsims <- 1000
m <- 13 # number of methods used. Allows for less coded needing to be written every time I change the # of methods
...
# table <- read.csv("sim study/raw results/table_ss_no dith_2021-08-04_520_run_1000_sims.csv")
...
for(i in 1:nsims){
  ...
  for(j in 1:length(paras)){
    ...
    y <- round(beta0 + beta1*x1 + beta2*x2 + e*(alpha_y + alpha_x1*x1), 0)
    ...
    for(k in 1:length(taus)){
      ...
      ## iid
      iid_beta <- summary(rq_model, se = "iid")[[3]][2,1]
      iid_se <- summary(rq_model, se="iid")[[3]][2,2]
      ...
      ## ker
      # ker_beta <- summary(rq_model, se = "ker")[[3]][2,1]
      # ker_se <- summary(rq_model, se="ker")[[3]][2,2]
      ...
      ## boot, pwy
      # pwy_coef <- summary(rq_model, se = "boot", bsmethod ="pwy", R = boot)[[3]][2,1:2]
      # pwy_beta <- pwy_coef[1]; pwy_se <- pwy_coef[2]
      
      ## boot, mcmb
      # skip_to_next <- F
      # tryCatch( mcmb_coef <- summary(rq_model, se = "boot", 
      #                                bsmethod ="mcmb", R = boot)[[3]][2,1:2]
      #           , error = function(e) { skip_to_next <<- T} )
      # if(skip_to_next == T) {next} # Prevents loop from stopping when this code fails. 
      # 
      # mcmb_beta <- mcmb_coef[1]; mcmb_se <- mcmb_coef[2]
      
      ## boot, wxy
      # wxy_coef <- summary(rq_model, se = "boot", bsmethod ="wxy", R = boot)[[3]][2,1:2]
      ...
      # bayes_adj_beta <- bayes_adj_sw[1]
      # bayes_adj_lb <- bayes_adj_sw[2]; bayes_adj_ub <- bayes_adj_sw[3]
      ...
      table[skip +11,1] <- "pbs"
      table[skip +12,1] <- "bayes"; table[skip +13,1] <- "bayes_adj" 
      ...
      # table[skip +13,2] <- bayes_adj_beta
...
#save as a csv file
write.csv(table, row.names = F, file = paste0("sim study/raw results/table_ss_no dith_v2", "_",  Sys.Date(), "_", nsims, "_sims.csv", sep = ""))
```
...
paras <- list( c(1, 100), c(1, 250), # Model 1: IID, normal x and errors
               c(2, 100), c(2, 250),# Model 2: NID, normal x and errors
               c(4, 100), c(4, 250)) # Model 4: NID, normal x, emg errors
taus <- c(0.1, 0.3, 0.5, 0.7, 0.9)
